  ...
  return (
    <Link
      ...
      className="group relative flex flex-col gap-3 rounded-2xl border border-border bg-card p-4 shadow-sm
                 sm:flex-col
                 max-sm:flex-row max-sm:items-center max-sm:rounded-xl max-sm:px-3 max-sm:py-2 max-sm:shadow-none max-sm:border-transparent"
    >
      {/* Avatar */}
      <div
        className="flex size-12 shrink-0 items-center justify-center rounded-full bg-secondary/30 text-xl
      ...
      <div className="min-w-0 flex-1">
        ...
        {league.location && (
          <p className="mt-0.5 truncate text-muted-foreground max-sm:hidden">
            {league.location}
          </p>
        )}
      </div>
